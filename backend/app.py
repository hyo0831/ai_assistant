...
트레이딩 모드(engine/) + 분석 모드(analysis/) + ORION 진단(orion/)을 하나의 API로 제공
배포 루트: backend/
"""
...
# - ENGINE_DIR: from core.data_fetcher import ... (트레이딩 모드)
...
# 필요한 디렉토리 미리 생성
for d in [PROJECT_ROOT / "results", PROJECT_ROOT / "feedback"]:
...
from analysis.ai_client import analyze_fundamentals_with_gemini
...
async def lifespan(app: FastAPI):
    ...
    print(f"  - 백엔드 루트: {PROJECT_ROOT}")
    print(f"  - 엔진 모듈: {ENGINE_DIR}")
    yield
    print("[OK] 서버 종료")
...
class OrionRequest(BaseModel):
    app_key: str = Field(..., description="KIS App Key")
    app_secret: str = Field(..., description="KIS App Secret")
    ...
    survey: dict = Field(default={}, description="설문 응답")


@app.post("/api/orion/analyze")
async def orion_analyze(request: OrionRequest):
    """ORION 투자 진단 — KIS API + OpenAI GPT-4o"""
    from orion.main import KISClient, process_trades, ai_analyze
    ...
    try:
        ...
        analysis, score, tier = ai_analyze(metrics, request.survey, request.openai_key)
        return {
            "metrics": metrics,
            "score": score,
            "tier": tier,
        ...
        raise HTTPException(status_code=500, detail=f"ORION 분석 오류: {str(e)}")
