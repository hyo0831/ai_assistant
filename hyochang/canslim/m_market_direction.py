...
def _analyze_index(ticker: str, name: str) -> Dict:
    ...
    try:
        ...
        if len(closes) >= 60:  # 3개월 데이터로 200일 MA 근사
            # 200일 MA는 별도 다운로드 필요
            try:
...
def format_for_prompt(data: Dict) -> str:
    ...
    # 종합 경고
    sp_dist = data.get('sp500', {}).get('distribution_days_5wk', 0)
    nq_dist = data.get('nasdaq', {}).get('distribution_days_5wk', 0)
    max_dist = max(sp_dist, nq_dist)
    if max_dist >= 5:
